...
export function AssetLibraryView() {
  ...
  const filter = React.useMemo<LibraryFilter>(
    () => ({
      ...parseLibraryFilter(new URLSearchParams(query)),
      resourceType: "brand",
    }),
    [query],
  );
  ...
  const downloadLabel = formatFilter
    ? t("downloadSelectedFormat", { format: formatFilter.toUpperCase() })
    : t("downloadSelectedAllFormats");
    ...
    <div className="space-y-6">
      ...
      {loadError ? (
        ...
        <>
          ...
          {formatFilter ? null : (
            <p className="text-sm text-muted-foreground">{t("downloadSelectedHint")}</p>
          )}
