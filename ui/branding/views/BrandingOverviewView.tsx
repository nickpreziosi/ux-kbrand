...
import { PageHeader, Tile } from "@k-lab/components";
import type { LucideIcon } from "lucide-react";
import { BookOpen, Image as ImageIcon, Palette, PenTool, Type } from "lucide-react";
...
export function BrandingOverviewView() {
  ...
  const { assets, loading } = usePublicAssets("brand-guidelines");
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="space-y-4" aria-label={t("sectionsAriaLabel")}>
      ...
      <section className="space-y-4" aria-label={t("sourceOfTruthAriaLabel")}>
        ...
        <DocumentViewerCard asset={brandBook} loading={loading} showPreview={false} />
      </section>
    </div>
  );
}
