...
export function ImageryView() {
  ...
  const { assets, loading, loadError } = usePublicAssets("brand-imagery");
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="space-y-4" aria-label={t("libraryTitle")}>
        <div>
          <h2 className="text-xl font-semibold">{t("libraryTitle")}</h2>
          <p className="text-sm text-muted-foreground">{t("libraryDescription")}</p>
        </div>
        {loadError ? (
